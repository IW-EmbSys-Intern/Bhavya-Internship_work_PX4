...
class CircleROS2(Node):
    ...
    def loop(self):
        ...
        # Figure-8 trajectory
        x = self.center_x + self.radius * math.sin(self.angle)
        y = self.center_y + 0.5 * self.radius * math.sin(2.0 * self.angle)
        z = self.altitude

        # Tangential velocity
        vx = self.radius * self.angular_speed * math.cos(self.angle)

        vy = (
            self.radius *
            self.angular_speed *
            math.cos(2.0 * self.angle)
        )
